...
n = 4
movement = np.array([(0, 1), (1, 0), (1, 1), (1, -1)])

largest = 0

for r in movement:
    i = np.multiply(np.arange(n).reshape((-1, 1)), r)

    for index, val in np.ndenumerate(a):
        ind = i + index
        if np.all(np.greater_equal(ind[-1], (0, 0))) and np.all(np.less(ind[-1], a.shape)):
            product = np.prod(a[ind.T[0], ind.T[1]])
            if product > largest:
                largest = product
